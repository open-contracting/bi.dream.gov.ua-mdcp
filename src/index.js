...
const main = async () => {
  ...
  requireTag.loaded = new Promise((resolve, reject) => {
      requireTag.onload = (response) => {
          window.require ? resolve(): reject();
      };
      requireTag.onerror = (error) => {
          reject(error);
      };
  });

  console.log("requireTag.loaded: ", requireTag.loaded);

  try { 
    await requireTag.loaded;
    }
  catch {
    return window.location.href = process.env.REACT_APP_QLIK_AUTH_PROXY_URL; 
    }
  ...
  window.require(['js/qlik'], function (qlik) {
    ...
    window.require(['url-util'], function(urlUtils){
      (async function init() {
        ...
        const qlikGlobal = await qlik.getGlobal(config); 
        ...
        qlikGlobal.getAuthenticatedUser(function(reply){
          const isUser =  reply.qReturn;
          console.log(reply.qReturn);
          if (isUser !== null) {
            console.log('User is authenticated:', isUser);
            ...
            app.bookmark.apply('b2213e6c-7dcf-4c83-91cf-b10da0a70bc6');
